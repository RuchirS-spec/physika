...
# DIST_SCALAR_AXIOM = {
#     "Normal":    ("Real.sample_normal",    2),
#     "Uniform":   ("Real.sample_uniform",   2),
#     "Beta":      ("Real.sample_beta",      2),
#     "Gamma":     ("Real.sample_gamma",     2),
#     "Bernoulli": ("Real.sample_bernoulli", 1),
# }

# def _elaborate_dual_sample(node: tuple, fvar_env: Dict[str, Expr],
#                            elab: Elab, errors: List[str]
#                            ) -> Optional[Expr]:
#     """
#     Elaborate a ``dual_sample`` node
#     (``name1 : T1, name2 : T2 ~ Dist(args, "score-function")``) to a
#     ``Prod``-typed term pairing sample with its log-probability.

#     Scoped to Bernoulli only — ELF's own ``bernoulli_dist`` notes
#     it's sole non-continuous distribution in set and only
#     one ever routed through score-function/dual_sample mode in practice.
#     Returns ``None`` (caller degrades to MVar) for any other distribution
#     or a malformed call.
...
#         AST node.
#     fvar_env : Dict[str, Expr]
#         Names currently in scope, forwarded to ``elaborate_expr``.
#     elab : Elab
#         Elaborator providing env/lctx/mctx and MVar information.
#     errors : List[str]
#         List forwarded to ``elaborate_expr``.
...
#     Example
#     -------
#     >>> from physika.utils.cic_utils.elab_utils import _elaborate_dual_sample
#     >>> from physika.core.elab.elab import Elab
#     >>> from physika.core.environment import Environment
#     >>> elab = Elab(Environment())
#     >>> call_node = ("call", "Bernoulli", [("num", 0.5)])
#     >>> node = ("dual_sample", "b", "ℝ", "lp", "ℝ", call_node)
#     >>> _elaborate_dual_sample(node, {}, elab, [])
#     App(func=Const(name='Real.sample_logprob_bernoulli', levels=()), arg=Lit(val=0.5))
#     """
#     from physika.core.elab.body_elab import elaborate_expr

#     from physika.features.randomness import extract_dist_args
#     _, _name1, _type1, _name2, _type2, call_node = node
#     if not (isinstance(call_node, tuple) and call_node
#             and call_node[0] == "call" and call_node[1] == "Bernoulli"):
#         return None
#     raw_args = call_node[2] if len(call_node) > 2 else []
#     param_args, shape_args, _mode = extract_dist_args(raw_args, 1)
#     if len(param_args) != 1:
#         return None
#     p_cic = elaborate_expr(param_args[0], fvar_env, elab, errors)
#     if isinstance(p_cic, MVar):
#         return None
#     if shape_args:
#         n_cic = elaborate_expr(shape_args[0], fvar_env, elab, errors)
#         if isinstance(n_cic, MVar):
#             return None
#         return App(App(Const("Vec.sample_logprob_bernoulli", ()), n_cic), p_cic)
#     return App(Const("Real.sample_logprob_bernoulli", ()), p_cic)

# def _try_elaborate_tabulate_construction(var_name: str, type_spec,
#                                          next_stmt: Optional[tuple],
#                                          fvar_env: Dict[str, Expr],
#                                          elab: Elab, errors: List[str]
#                                          ) -> Optional[Expr]:
#     """
#     Recognize ``arr : ℝ[d1, ..., dk]`` (no initializer) immediately
#     followed by a multi-index loop that writes every cell of ``arr``
#     exactly once, and elaborate it as nested ``Vec.tabulate`` over
#     ``i1...ik``.

#     Matches either ``for i1 ... ik: arr[i1,...,ik] = expr`` (parsed as
#     ``body_for_map``) or same shape with ``+=`` (``body_for_accum``,
#     since starting from implicit zero this behaves identically to
#     ``=`` when there's no extra index).

#     Also recognizes genuine-reduction shape ``body_for_accum`` with
#     more loop variables than ``arr`` has dimensions (e.g.
#     ``for i j k: C[i,j] += A[i,k] * B[k,j]``, matrix multiplication):
#     leftover trailing loop variable(s) become a ``Vec.foldl``-based
#     sum nested inside output-dimension ``Vec.tabulate``s, with the
#     reduction bound recovered via ``infer_reduction_bound``. Scoped to
#     at most one reduction variable — a second one falls through
#     undetected, same "decline rather than guess" contract as every other
#     ``_try_elaborate_*`` helper.

#     Only a single assignment statement in loop body is recognized;
#     loop's own index list must exactly match ``i1...ik`` (and, for
#     reduction case, trailing variable(s)) in declaration order,
#     same "imaginary" (outermost) / named (inner) convention every
#     other loop-body index reference already uses.
...
#         array local's name.
#     type_spec : Union[str, tuple]
#         ``var_name``'s declared type spec; only ``("tensor", dims)``
#         matches.
#     next_stmt : Optional[tuple]
#         statement immediately following ``var_name``'s declaration —
#         matches only ``body_for_map``/``body_for_accum``.
#     fvar_env : Dict[str, Expr]
#         Names currently in scope, used to resolve any dim vars in
#         ``type_spec`` and reduction bound.
#     elab : Elab
#         Elaborator providing env/lctx/mctx and FVar information.
#     errors : List[str]
#         List forwarded to ``elaborate_expr``.
...
#     Example
#     -------
#     >>> from physika.utils.cic_utils.elab_utils import _try_elaborate_tabulate_construction  # noqa: E501
#     >>> from physika.core.elab.elab import Elab
#     >>> from physika.core.environment import Environment
#     >>> elab = Elab(Environment())
#     >>> type_spec = ("tensor", [(2, "invariant")])
#     >>> loop_body = [("loop_index_assign_nd", "arr",
#     ...               [("index_item", ("imaginary",))], ("num", 1.0))]
#     >>> next_stmt = ("body_for_map", ["i"], loop_body)
#     >>> result = _try_elaborate_tabulate_construction(
#     ...     "arr", type_spec, next_stmt, {}, elab, [])
#     >>> result
#     App(func=App(func=App(func=Const(name='Vec.tabulate', levels=()), arg=Const(name='Real', levels=())), arg=Lit(val=2)), arg=Lam(binder_name='i', binder_type=App(func=Const(name='Fin', levels=()), arg=Lit(val=2)), body=Lit(val=1.0), binder_info=<BinderInfo.DEFAULT: 1>))
#     """
#     from physika.core.elab.body_elab import elaborate_expr
#     from physika.core.elab.dim_typespec import dim_to_cic_resolved

#     if not (isinstance(next_stmt, tuple) and next_stmt
#             and next_stmt[0] in ("body_for_map", "body_for_accum")):
#         return None
#     if not (isinstance(type_spec, tuple) and type_spec[0] == "tensor"):
#         return None
#     loop_vars, loop_body = next_stmt[1], next_stmt[2]
#     if not isinstance(loop_body, list) or len(loop_body) != 1:
#         return None
#     assign_stmt = loop_body[0]
#     if not (isinstance(assign_stmt, tuple) and len(assign_stmt) == 4
#             and assign_stmt[0] in ("loop_index_assign_nd", "loop_index_pluseq")
#             and assign_stmt[1] == var_name):
#         return None
#     is_pluseq = assign_stmt[0] == "loop_index_pluseq"
#     index_items, rhs_node = assign_stmt[2], assign_stmt[3]
#     dims = type_spec[1]
#     num_out = len(dims)
#     if len(index_items) != num_out or len(loop_vars) < num_out:
#         return None
#     reduction_vars = loop_vars[num_out:]
#     if reduction_vars and not is_pluseq:
#         return None  # a "=" with a leftover loop var isn't this idiom
#     if len(reduction_vars) > 1:
#         return None  # scope: at most one reduction dimension
#     output_vars = loop_vars[:num_out]
#     # Verify write target's own index list is exactly output_vars,
#     # in order, using same imaginary(outermost)/named(inner)
#     # convention as everywhere else — anything else (a permuted or
#     # partial index list) isn't a shape this recognizes.
#     for pos, item in enumerate(index_items):
#         if not (isinstance(item, tuple) and len(item) == 2
#                 and item[0] == "index_item"):
#             return None
#         inner = item[1]
#         if pos == 0:
#             if not (isinstance(inner, tuple) and inner and inner[0] == "imaginary"):
#                 return None
#         else:
#             if not (isinstance(inner, tuple) and inner
#                     and inner[0] == "var" and inner[1] == output_vars[pos]):
#                 return None
#     dim_exprs = [dim_to_cic_resolved(d[0], fvar_env.get) for d in dims]
#     if any(d is None for d in dim_exprs):
#         return None
#     # Open one Fin-typed local per output dimension, deepest last — key
#     # "i" for outermost regardless of its own name (matching the
#     # lexer's own hardcoded "imaginary" convention every other loop-body
#     # index reference already relies on).
#     level_env, level_elab, fvs = dict(fvar_env), elab, []
#     for pos, ov in enumerate(output_vars):
#         fin_d = App(Const("Fin", ()), dim_exprs[pos])
#         level_elab, fv = level_elab.with_local(ov, fin_d)
#         level_env = {**level_env, ("i" if pos == 0 else ov): fv}
#         fvs.append(fv)
#     if reduction_vars:
#         red_var = reduction_vars[0]
#         red_bound = infer_reduction_bound(rhs_node, red_var, level_env,
#                                            level_elab)
#         if red_bound is None:
#             return None
#         fin_red = App(Const("Fin", ()), red_bound)
#         level_elab, red_fv = level_elab.with_local(red_var, fin_red)
#         level_env = {**level_env, red_var: red_fv}
#         level_elab, acc_fv = level_elab.with_local("_acc", _REAL_CONST)
#         level_env = {**level_env, "_acc": acc_fv}
#         term_cic = elaborate_expr(rhs_node, level_env, level_elab, errors,
#                                    expected_type=_REAL_CONST)
#         if isinstance(term_cic, MVar):
#             return None
#         step_body = App(App(Const("Real.add", ()), acc_fv), term_cic)
#         fold_fn = level_elab.state.lctx.mk_lambda([red_fv, acc_fv], step_body)
#         innermost = App(App(App(App(Const("Vec.foldl", ()), _REAL_CONST),
#                                 red_bound), fold_fn), Lit(0.0))
#     else:
#         body_cic = elaborate_expr(rhs_node, level_env, level_elab, errors,
#                                    expected_type=_REAL_CONST)
#         if isinstance(body_cic, MVar):
#             return None
#         innermost = body_cic
#     result = innermost
#     # Vec.tabulate's own element type (its first, {α}, argument) is
#     # threaded from inside out: innermost value is always Real,
#     # but each successive *outer* Vec.tabulate wraps previous one,
#     # so its element type is that previous Vec.tabulate's own result
#     # type (Vec Real d), not Real again — a 2D construction's outer
#     # tabulate has element type "Vec Real m", never bare Real.
#     elem_type: Expr = _REAL_CONST
#     for pos in range(len(output_vars) - 1, -1, -1):
#         fn = level_elab.state.lctx.mk_lambda([fvs[pos]], result)
#         result = App(App(App(Const("Vec.tabulate", ()), elem_type),
#                          dim_exprs[pos]), fn)
#         elem_type = App(App(Const("Vec", ()), elem_type), dim_exprs[pos])
#     return result
